...
class AlphaVantage {
    ...
    async getRSI(dates, threshold, instrument) {
        try {

            await alpha.technical.rsi(instrument, `daily`, 14, `close`).then(data => {
                const array = Object.entries(data);
                const objsArray = array[1][1]
                // const rsi = parseFloat(objsArray[dates[0]].RSI)
                if (parseFloat(objsArray[dates[0]].RSI) < threshold
                    || parseFloat(objsArray[dates[1]].RSI < threshold
                        || parseFloat(objsArray[dates[2]].RSI)) < threshold
                    || parseFloat(objsArray[dates[3]].RSI) < threshold) {
                    console.log(`Pattern for ${instrument}`)
                }
            });
        }
        catch (e) {

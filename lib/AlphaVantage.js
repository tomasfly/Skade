...
class AlphaVantage {
    ...
    getRSI(instrument) {
        alpha.technical.rsi(instrument, `daily`, 14, `low`).then(data => {
            const array = Object.entries(data);
            const objsArray = array[1][1]
            let counter = 0
            Object.entries(objsArray).forEach(([key, value]) => {
                if (counter === 0) {
                    console.log(`Printing RSI values for ${instrument}`)
                    console.log(JSON.stringify(key) + ' ' + JSON.stringify(value.RSI));
                }
                counter++
            });
        });
    ...
    async getMACDDifference(instrument) {
        let difference;
